...
setup_gcc() {
	...
	if is_true "${USE_CUSTOM_GCC_64:-false}"; then
		...
		# Ensure aarch64-linux-android-nm exists (some AOSP tags only ship versioned binary)
		if [ ! -x "${GCC64_DIR}/bin/aarch64-linux-android-nm" ]; then
			real_nm=$(find "${GCC64_DIR}/bin" -name 'aarch64-linux-android-nm*' -type f -executable -print -quit 2>/dev/null || true)
			if [ -n "$real_nm" ]; then
				ln -sf "$real_nm" "${GCC64_DIR}/bin/aarch64-linux-android-nm"
				echo "linked $real_nm -> aarch64-linux-android-nm"
			fi
		fi

		# Put gcc-64/bin at the FRONT of PATH so bare 'nm'/'as'/'ld' resolve to
		# the cross toolchain, not /usr/bin. This fixes the vDSO nm '-O' error
